...
(function ($, window) {
    ...
    jQuery.fn.workflow = function (xmlContent, settings) {
        ...
        var _initData = function () {
            ...
            // 生成拖拽框
            var mainDiv = $('<div></div>');
            mainDiv.attr("style", "width: 1024px; height: 768px; background-color: rgb(216, 216, 216); border: 1px solid #C0C0C0; margin: 0 auto; position: relative; padding: 0;z-index:9;");
            ...
            workflowDiv.attr("style", "width: 100%; height: 720px; background-color: white; margin: 0; padding: 0; z-index:10;");
            ...
            propertyDiv.attr("style", "width: 1024px;; bottom: 0px;display:none; background: white; position: absolute; top: 678px;left:-200px; margin: 0; padding: 0;");
            ...
            'overflow-y:hidden;border:1px solid #919999; z-index: 1000;filter:alpha(opacity=20);opacity: 0.8; ' +
            ...
            for (var index = 0; index < plugins.length; index++) {
              ...
              //  plugin['properties']['lable'];
                var pluginNode = $('<img src="' + image + '" draggable="true" width="' + config.nodeWidth + '" height="' + config.nodeHeight + '"  style="border:1px solid #c3c3c3;vertical-align: middle; margin-top:8px;" id="' + type + '"><label style="font-size: 12px; font-weight: 700;width: 50px; display:inline-block;">' + plugin['properties'].label + '&nbsp;</label>');
            ...
            //$("#rightClickMenuDiv ul li").css("color","red");
        };
            ...
            if (ev.button == 0) {
                ...
                if (selectedNode == null) {
                    ...
                    $("#deleteLi").click(function () {
                        menuDeleteNote(evx, evy);
                    });
                    $("#copyLi").click(function () {
                        menuCopyNote(selectedNode);
                    });
            ...
            if (ev.button == 0) {
                ...
                if (selectedNode == null) {
                   ...
                   // alert($("#propertyDivTable").height());
                    ...
                    $("#propertyDiv").css("top", 691-$("#propertyDivTable").height() + "px");
                    $("#propertyDiv").show();
        ...
        _self.changeLineLabel = function (startNodeName,endNodeName,content) {
            $(globalParam.xmlContent).find("hops").children("hop").each(function (hopIndex, hop) {     //查找所有nodes节点并遍历
               if(startNodeName == $(hop).children("from").text() && endNodeName == $(hop).children("to").text()){
                   $(hop).children("properties").children("label").text(content);
                   canvasContentDraw();
               }
            });
        }
        ...
        // 右击菜单删除节点
        var menuDeleteNote = function (evx, evy) {
            var selectedNode = getClickNode(evx, evy);
            //删除一个节点以及这个节点上的连线
            var name = $(selectedNode).children("name").text();
            deleteNode(name);
        ...
        // 右击菜单复制
        var menuCopyNote = function (selectedNode) {
            globalParam.copyNode = selectedNode;
            $("#rightClickMenuDiv").hide();
        }
        // 右击菜单粘贴
            ...
            },
            {
                ...
                getXmlContent: function () {
                    ...
                    var nodeString =
                        ...
                        '<properties>' +
                        '<label readonly="false" type="text" title="节点名称" description="节点名称">' + this.type + '</label>\n' +
                        '<file readonly="true" type="file" title="启动文件" description="选择一个hive-sql文件"></file>\n' +
                        '<attempt readonly="false" type="text" title="重试次数" description="填写数字">0</attempt>\n' +
                        '</properties>' +
                        '</node>';
                    return nodeString;
            ...
            },
            {
                ...
                getXmlContent: function () {
                    ...
                    var nodeString =
                        ...
                        '<properties>' +
                        '<label readonly="false" type="text" title="节点名称" description="节点名称">' + this.type + '</label>\n' +
                        '<file readonly="true" type="file" title="启动文件" description="选择一个spar-sql文件"></file>\n' +
                        '<attempt readonly="false" type="text" title="重试次数" description="填写数字">0</attempt>\n' +
                        '</properties>' +
                        '</node>';
                    return nodeString;
